...
namespace Ts3Bot
{
    public class Worker : BackgroundService
    {
        ...
        protected override async Task ExecuteAsync(CancellationToken stoppingToken)
        {
            ...
            string ts3Host = Environment.GetEnvironmentVariable("TS3_HOST") ?? configuration["Ts3:Host"] ?? "localhost";
            string ts3User = Environment.GetEnvironmentVariable("TS3_USER") ??
                             configuration["Ts3:Username"] ?? "serveradmin";
            string ts3Pass = Environment.GetEnvironmentVariable("TS3_PASS") ?? configuration["Ts3:Password"] ?? "";

            logger.LogInformation("Worker started at: {Time}", DateTimeOffset.Now);

            logger.LogInformation("Trying to Connect to {Host}", ts3Host);

            TeamSpeakClient rc = new TeamSpeakClient(ts3Host);
            await rc.Connect();
            ...
            List<int> frmdGroups = serverGroups
                .Where(g => g.Name == "FRMD")
                .Select(g => g.Id)
                .ToList();
